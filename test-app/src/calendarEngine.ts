...
/**
 * Returns number of days in a given Gregorian month.
 */
...
/**
 * Returns number of days in a given Solar Hijri month.
 */
export function getSolarHijriDaysInMonth(year: number, month: number): number {
...
/**
 * Returns number of days in a given Tabular Lunar Hijri month.
 */
export function getLunarHijriDaysInMonth(year: number, month: number): number {
    if (month === 12) return isLunarHijriLeapYear(year) ? 30 : 29;
 ...
 * @param locale - Current UI language ('en' | 'fa' | 'ps').
 * @param numberSystem - Explicit digit style: 'latin' (123) or 'persian' (۱۲۳).
 *   English always uses 'latin'. For fa/ps this is user-configurable.
 */
export function formatNumber(
    num: number | string,
    locale: 'en' | 'fa' | 'ps' | 'dr',
    numberSystem: 'latin' | 'persian' = 'latin'
): string {
    const numericValue = typeof num === 'string' ? parseFloat(num.replace(/[^\d.-]/g, '')) : num;
    if (isNaN(numericValue)) return num.toString();

    // English always uses Western Arabic regardless of the system toggle.
    if (locale === 'en' || numberSystem === 'latin') {
        return new Intl.NumberFormat('en-US', { useGrouping: false }).format(numericValue);
    }

    // fa-IR natively provides Eastern Arabic/Persian digits (۰–۹).
